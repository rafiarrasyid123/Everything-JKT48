// Sistem Authentication Enhanced untuk Website Mikaela
// Dengan kemampuan auto-sync ke accounts.json untuk cross-device compatibility
class AuthSystemEnhanced {
    ...
    async loadUsers() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                users = data.users || [];
                console.log('✅ Loaded', users.length, 'users from accounts.json');
            }
        } catch (error) {
    ...
    saveUsers() {
        ...
        // Secondary storage: sync dengan accounts.json
        this.syncWithAccountsJson();
    }

    // Sync dengan accounts.json untuk cross-device compatibility
    async syncWithAccountsJson() {
        try {
            console.log('🔄 Syncing user data with accounts.json...');

            // Buat backup terlebih dahulu
            const backupResult = await this.createBackup();
            if (!backupResult.success) {
                console.warn('⚠️ Backup failed, but continuing with sync...');
            }

            // Update accounts.json dengan data terbaru
            const syncData = {
                users: this.users,
                metadata: {
                    version: "2.0",
                    ...
                    syncedFrom: "auth-enhanced.js",
                    deviceInfo: this.getCurrentDeviceInfo(),
                    backupAvailable: backupResult.success
                }
            };

            // Simpan ke localStorage sebagai cache
            localStorage.setItem('mikaela_accounts_sync', JSON.stringify(syncData));

            // Update file accounts.json (menggunakan teknik yang kompatibel dengan browser)
            const updateResult = await this.updateAccountsJsonFile(syncData);

            if (updateResult.success) {
                console.log('✅ Successfully synced with accounts.json');
                this.showNotification('✅ Data berhasil di-sync ke accounts.json', 'success');
                return { success: true, message: 'Data berhasil di-sync dengan accounts.json' };
            } else {
                console.warn('⚠️ File sync failed, but data saved to localStorage:', updateResult.message);
                this.showNotification('⚠️ Data tersimpan lokal, sync file gagal', 'error');
                return { success: false, message: 'Gagal sync ke file: ' + updateResult.message };
            }

        } catch (error) {
            console.error('❌ Sync with accounts.json failed:', error);
            this.showNotification('❌ Sync gagal: ' + error.message, 'error');
            return { success: false, message: 'Gagal sync: ' + error.message };
        }
    }

    // Update file accounts.json
    async updateAccountsJsonFile(syncData) {
        try {
            // Karena browser security, kita akan menggunakan teknik yang berbeda
            // 1. Simpan sebagai file terpisah yang bisa didownload
            // 2. Atau gunakan localStorage sebagai primary dengan fallback

            const jsonString = JSON.stringify(syncData, null, 2);

            // Teknik 1: Simpan sebagai blob dan download
            ...
            downloadLink.download = 'accounts.json';
            ...
            // Teknik 2: Update localStorage dengan metadata
            localStorage.setItem('mikaela_accounts_json', jsonString);
            localStorage.setItem('mikaela_last_accounts_update', new Date().toISOString());

            return {
                success: true,
                message: 'File accounts.json berhasil diupdate',
                method: 'download_trigger'
            };
        ...
        try {
            ...
            const backupData = {
                ...
                metadata: {
                    ...
                    description: "Backup before accounts.json update"
                }
            };

            const backupJson = JSON.stringify(backupData, null, 2);
            const backupFilename = `accounts-backup-${timestamp}.json`;

            // Simpan backup ke localStorage
            localStorage.setItem(`mikaela_backup_${timestamp}`, backupJson);

            // Juga buat file backup
    ...
    // Register user baru dengan auto-sync ke accounts.json
    register(username, email, password) {
        ...
        // Auto-sync dengan accounts.json setelah register berhasil
        this.syncWithAccountsJson().then(syncResult => {
            console.log('📝 Auto-sync after registration:', syncResult.message);
            if (syncResult.success) {
                this.showNotification('✅ Akun berhasil dibuat dan di-sync!', 'success');
            } else {
                this.showNotification('✅ Akun berhasil dibuat, tapi sync file gagal', 'success');
            }
        }).catch(error => {
            console.log('⚠️ Auto-sync after registration failed:', error);
            this.showNotification('✅ Akun berhasil dibuat, sync akan dicoba lagi nanti', 'success');
        });
    ...
    saveToUserFiles() {
        // Save individual user data for better compatibility
        this.users.forEach(user => {
            localStorage.setItem(`mikaela_user_${user.id}`, JSON.stringify(user));
        });
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    window.auth = new AuthSystemEnhanced();
});
